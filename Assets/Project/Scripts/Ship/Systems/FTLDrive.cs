...
namespace SpaceSurvivor.Ship
{
    ...
    public class FTLDrive : NetworkBehaviour, IPowerConsumer, IRepairable
    {
        ...
        public void SetPowerState(bool isOn)
        {
            ...
            {
                CancelCharge();
                LogVWarn("[FTLDrive] Blackout durante la carica — salto annullato, lockout 30s");
            }
        }
        ...
        void IRepairable.ApplyRepair(float progressPercent)
        {
            ...
            LogV($"[FTLDrive] Repair {progressPercent}% → HP {_netHealth.Value:F0}/{_data.maxHealth}");
        }
        ...
        private void TryInitiateJumpInternal()
        {
            ...
            {
                LogVWarn($"[FTLDrive] Salto negato — stato: {CurrentState}");
                return;
            }

            if (!_isPowered)
            {
                LogVWarn("[FTLDrive] Salto negato — sistema non alimentato");
                return;
            }

            if (HealthToState(CurrentHealthPercent) == ShipSystemState.Offline)
            {
                LogVWarn("[FTLDrive] Salto negato — sistema OFFLINE, riparare prima");
                return;
            }
        ...
        private void ApplyDamageInternal(float amount)
        {
            ...
            {
                CancelCharge();
                LogVWarn("[FTLDrive] Sistema portato OFFLINE durante la carica — salto annullato");
            }
        }
        ...
        private IEnumerator ChargeRoutine()
        {
            ...
            LogV($"[FTLDrive] Carica FTL avviata ({duration}s, {GetPowerDemand():F0}W)...");
        ...
        private IEnumerator JumpRoutine()
        {
            ...
            LogV("[FTLDrive] SALTO FTL in corso...");

            float jumpDuration = _data?.jumpTransitionDuration ?? 2.5f;
            yield return new WaitForSeconds(jumpDuration);

            // Salto completato
            LogV("[FTLDrive] Salto completato. Cooldown avviato.");
        ...
        {
            LogV($"[FTLDrive] Stato → {(FTLState)curr}");
        }
        ...
        private void ApplyUpgradeInternal(int tierIndex)
        {
            ...
            LogV($"[FTLDrive] Upgraded to {_data.displayName}");
        }
        ...
        [Tooltip("Log diagnostici verbosi (carica/salto/stati/upgrade FTL). Standard Rev BA — default off.")]
