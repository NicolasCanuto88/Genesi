...
namespace SpaceSurvivor.Ship
{
    ...
    ///
    ///   Soglie (50% / 75% / 100%):
    ///     Materiali consumati SOLO al superamento soglia — mai all'avvio,
    ///     mai in caso di interruzione (regola invariante GDD).
    ///
    ...
    public class RepairMinigame : MonoBehaviour
    {
        ...
        [Tooltip("Marker visivo a 50% sulla barra. Posizionalo manualmente in Canvas.")]
        [SerializeField] private RectTransform   marker50;
        [SerializeField] private RectTransform   marker75;
        ...
        // Soglie già superate in questa sessione (per non riconsumare materiali)
        ...
        private void Update()
        {
            ...
            if (_inGracePeriod)
            {
                ...
                else
                {
                    ...
                    // Non applicare decay, non controllare slider
                    UpdateUI();
                    return;
                }
            }
            ...
            if (_sliderActive)
            {
                ...
                {
                    // Timeout — indicatore uscito senza essere premuto
        ...
        /// <summary>Apre il minigame per il sistema specificato.</summary>
        public void Open(IRepairable target, Action onComplete, Action onInterrupted)
        {
        ...
        private IEnumerator SliderRoutine()
        {
            while (_isActive)
            {
                ...
                // Attendi che lo slider venga risolto (in Update o OnSliderKeyPerformed)
        ...
        private void OnTimerExpired()
        {
            ...
            // Materiali già consumati alle soglie precedenti NON vengono rimborsati
        ...
        private void CrossThreshold(float pct, ref bool flag)
        {
            ...
            // Consuma materiali per questa soglia
            ConsumeThresholdMaterials(pct);

            // Applica riparazione parziale/totale al sistema
            _target.ApplyRepair(pct);
        ...
        /// <summary>
        /// Consuma i materiali associati alla soglia specificata.
        /// Regola invariante: consuma SOLO al completamento soglia.
        /// </summary>
        private void ConsumeThresholdMaterials(float pct)
        {
            if (InventorySystem.Instance == null) return;

            var thresholds = _target.GetRepairThresholds();
            if (thresholds == null) return;

            foreach (var threshold in thresholds)
            {
                if (!Mathf.Approximately(threshold.progress * 100f, pct)) continue;

                foreach (var req in threshold.materials)
                {
                    bool consumed = InventorySystem.Instance.TryConsume(req.itemType, req.amount);
                    if (!consumed)
                        Debug.LogWarning($"[RepairMinigame] Materiali insufficienti: " +
                                         $"{req.amount}× {req.itemType} a soglia {pct}%");
                }
                break;
            }
        }
        ...
        private void UpdateSliderIndicatorPosition()
        {
            ...
            // Range effettivo: bordo sinistro → bordo destro, tenendo conto della larghezza dell'indicatore
            ...
            // Zona verde = sempre fissa, indica dove premere
            if (hitZoneImage != null)
                hitZoneImage.color = colorGood;

            // Indicatore cambia colore quando entra nella zona
        ...
        private string GetActionDisplayName(int index)
        {
            ...
            // Prende il nome del primo binding (keyboard o gamepad)
